...
```{r}
recommendation <- get_topn_recos(rating_matrix, 20)
recommendation <- recommendation %>% filter(user %in% c(5,25,50,150))
...
movies <- as(MovieLense, "data.frame")
movies <- movies %>% filter(user %in% c(5,25,50,150))
...
create_cleveland_plot_1 <- function(df, title) {
  ggplot(df, aes(reorder(genre, act), value), height = 500, width = 7) +
...
p1 <- create_cleveland_plot_1(reco_df %>% filter(user == 5), 'Top-20 Empfehlungen für Nutzer 5')
p2 <- create_cleveland_plot_1(reco_df %>% filter(user == 25), 'Top-20 Empfehlungen für Nutzer 25')
p3 <- create_cleveland_plot_1(reco_df %>% filter(user == 50), 'Top-20 Empfehlungen für Nutzer 50')
p4 <- create_cleveland_plot_1(reco_df %>% filter(user == 150), 'Top-20 Empfehlungen für Nutzer 150')
