...
```
Für diesen Nutzer funktioniert der Recommender fast gar nicht. Er empfielt nur zwei verschiedene Genres: Drama und Comedy. Beide werden von diesem Nutzer sehr häufig geschaut. Jedoch kommt das zweithäuffigste Genre Thriller gar nicht vor. Dies macht die Vorschläge für diesen Nutzer ziemlich eintönig.

```{r fig.width = 12, fig.height = 12}
show_cleveland_grid(555)
```
Hier sieht man Unterschiede zwischen den verschiedenen Top-N Empfehlungen. Allgemein gibt es den Trend, dass die Genres, welche am meisten geschaut werden zu viel empfohlen werden und die kaum geschauten Genres gar nicht vorgeschlagen werden. 
