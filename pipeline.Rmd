...
```
In dem MovieLense Datenset sind insgesamt 216  verschiedene Genreprofile von den Filmen vertreten. Das bedeutet, es sind 216 verschiedene Genrekombinationen aus den 16 gegebenen Genres entstanden. Hier sehen wir also, dass die häufigste Kombination `Drama` alleine ist. Ganze 370 Filme wirden nur als Dramas kategorisiert.

## 7.User-Genre-Profil Matrix mit Nutzerprofilen im Genre-Vektorraum erzeugen.
```{r}
user_genre_profil <- user_like_item %*% movie_genre_matrix
user_genre_profil[1:5,1:5]
```
```{r}
dim(user_genre_profil)
```
